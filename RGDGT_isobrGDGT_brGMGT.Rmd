...
#2. Name the columns at least by the following items (CAPsensitive, but not necessarily in this order): age; 1302; 1300; 1298; 1296; 1292; 1292'; 1050-1; 1050-2; H-1048; 1036-1; 1036-2; 1034-1; 1034-2; H-1034a; H-1034b; H-1034c; 1032-1; 1032-2; 1022; 1020; H-1020a; H-1020b; H-1020c; 1018; 1050-tot; 1036-tot; 1034-tot; H-1034tot; 1032-tot.
...
#5. When you have not analyzed certain elements in your entire dataframe (e.g., the brGMGTs), add NA, or leave those cells blank. When you have analyzed it and it was not in there, add 0 for those components.
...
#some sums
          ...
          `SUM.Hbr` = `H-1048` + `H-1034tot` + `H-1020a` + `H-1020b` + `H-1020c`,
          `SUM.ALL` = SUM.ISO + SUM.BR + SUM.Hbr,
#fractional abundances for a total picture
          ...
          `FH-1048.ALL` = `H-1048` / SUM.ALL,
          `FH-1034tot.ALL` = `H-1034tot` / SUM.ALL,
          `FH-1020a.ALL` = `H-1020a` / SUM.ALL,
          `FH-1020b.ALL` = `H-1020b` / SUM.ALL,
          `FH-1020c.ALL` = `H-1020c` / SUM.ALL,
...
#deltaSST-MAT
          dT = SSTOBL - MAT5ME,
#and some brGMGT stuff. Fractional abundances:
          `FH1048.Hbr` = `H-1048` / `SUM.Hbr`,  
          `FH1034a.Hbr` = `H-1034a` / `SUM.Hbr`,
          `FH1034b.Hbr` = `H-1034b` / `SUM.Hbr`,
          `FH1034c.Hbr` = `H-1034c` / `SUM.Hbr`,
          `FH1034tot.Hbr` = `H-1034tot` / `SUM.Hbr`,
          `FH1020a.Hbr` = `H-1020a` / `SUM.Hbr`,
          `FH1020b.Hbr` = `H-1020b` / `SUM.Hbr`,
          `FH1020c.Hbr` = `H-1020c` / `SUM.Hbr`,
#%GMGTs. There are two types, one using all brGMGTs over all brGDGTs (Baxter et al., 2019):  
          `FHbr.BR` = SUM.Hbr / (SUM.Hbr + SUM.BR),
#and one using specific brGMGTs and only the 5-methyl, acyclic brGDGTs (Naafs et al., 2018): 
          `FHbr.BR.N` = (`H-1020c`+`H-1034b`+`H-1048`) / (`H-1020c`+`H-1034b`+`H-1048` + `1022` + `1036-1` + `1050-1`),
#Naafs et al., 2018 showed that the methylation of brGMGTs varies with temperature in peats as well. Again two types:
          `HMBT.ALL` = (`H-1020a` + `H-1020b` + `H-1020c`) / (`H-1020a` + `H-1020b` + `H-1020c` + `H-1034tot` + `H-1048`),
          `HMBT` = `H-1020c`/(`H-1020c`+ `H-1034b`+`H-1048`),
#Baxter et al., showed a strong correlation between the below index and tropical lake temperature:
          `brGMGTI` = (`FH1020c.Hbr`+ `FH1034a.Hbr` + `FH1034c.Hbr`) / (`FH1020b.Hbr` + `FH1020c.Hbr` + `FH1034a.Hbr` + `FH1034c.Hbr` + `FH1048.Hbr`)
              )
...
dat1 <- select(OGdata, age, `F1302.ALL`:`FH-1020c.ALL`)
dat1 <- gather(dat1, `F1302.ALL`:`FH-1020c.ALL`, value = "fraction", key = "biomarker")
dat1
 ggplot(dat1)+
      geom_area(mapping = aes(x = age, y = fraction, fill = biomarker))+
      scale_fill_manual(values = c("#003300", "#336633", "#009933", "#66CC33", "#66FF00", "#CCFF99", "#CCFFCC", "#000033", "#003366", "#003399", "#0033CC", "#3366FF", "#0099FF", "#660000", "#FF0000", "#FF66CC", "#FF6666", "#FFCCCC"))+
   Theme1
...
dat6 <- gather(dat6, "BIT", "IR", "NoRINGStetra", key = "TYPE", value = "index")
...
dat7r <- gather(dat7, `MAT5ME`: MAATPEAT, key = "TYPE", value = "Temp")
...
dat8sa <- gather(dat8s, SSTOBL, key = "Type", value = "Temp")
dat8ma <- gather(dat8m, MAT5ME, key = "Type", value = "Temp")
...
```{r}
# Now the H-shaped brGMGTs 
#First we select and transform the data frame
dat10 <- select(OGdata, MBT5ME, `MAT5ME`, `OUTLIERMBT`, `brGMGTI`, `outlierCAPRI`, `SSTOBL`, `FH1048.Hbr`, `FH1034a.Hbr`, `FH1034b.Hbr`, `FH1034c.Hbr`, `FH1020a.Hbr`, `FH1020b.Hbr`, `FH1020c.Hbr`)
dat10 <- filter(dat10, !MBT5ME==1)
dat10r <- gather(dat10, `FH1048.Hbr`:`FH1020c.Hbr`, value = "FA", key = "HbrGDGT")

#Look at the correlation of fractional abundances with the MAAT reconstructions from MBT5ME
ggplot()+
    geom_point(dat10r, mapping = aes(x = MAT5ME, y = FA, shape = OUTLIERMBT, color = OUTLIERMBT), size = 0.2)+
    scale_shape_manual(values = c(16,4))+
    scale_color_manual(values = c("black", "red"))+
    facet_grid(HbrGDGT~., scales = "free")+
    Theme1+
      theme(aspect.ratio = 0.5)

#removing the `#` in the command line below saves this figure as PDF in your RGDGT folder
ggsave("10a. HbrGMGT_MAAT.pdf")

#and then lok at the correlation with TEX86-based SST
ggplot()+
    geom_point(dat10r, mapping = aes(x = SSTOBL, y = FA, shape = outlierCAPRI, color = outlierCAPRI), size = 0.2)+
    scale_shape_manual(values = c(16,4))+
    scale_color_manual(values = c("black", "red"))+
    facet_grid(HbrGDGT~., scales = "free")+
    Theme1+
...
#removing the `#` in the command line below saves this figure as PDF in your RGDGT folder
ggsave("10b. HbrGMGT_SST.pdf")
```

```{r}
#Look at the correlation between brGMGTI' index and MAAT or SST

#some data selection and transformation
dat11 <- select(OGdata, SSTOBL, BIT, MBT5ME, MAT5ME, OUTLIERMBT, outlierCAPRI, `brGMGTI`)
MATGOOD <- filter(dat11, !OUTLIERMBT, !MBT5ME==1)
MATBAD <- filter(dat11, OUTLIERMBT)
SSTGOOD <- filter(dat11, !outlierCAPRI)
SSTBAD <- filter(dat11, outlierCAPRI)
MATGOOD <- gather(MATGOOD, MAT5ME, value = "temp", key = "proxy")
MATBAD <- gather(MATBAD, MAT5ME, value = "temp", key = "proxy")
SSTGOOD <- gather(SSTGOOD, SSTOBL, value = "temp", key = "proxy")
SSTBAD <- gather(SSTBAD, SSTOBL, value = "temp", key = "proxy")
GOOD <- bind_rows(MATGOOD, SSTGOOD)
BAD <- bind_rows(MATBAD, SSTBAD)

#then plotting the index against SST and MAAT
ggplot()+
  geom_point(GOOD, mapping = aes(x = `temp`, y = `brGMGTI`, color = BIT), size = 0.4)+
  geom_smooth(GOOD, mapping=aes(x=temp, y=brGMGTI), method="lm")+
  stat_cor(GOOD, mapping = aes(x=temp, y=brGMGTI))+
  geom_point(BAD, mapping = aes(x = `temp`, y = `brGMGTI`), shape = 4, color = "grey", size = 0.4)+
  scale_color_gradient(low = "light blue", high = "dark green")+
  facet_grid(.~proxy, scales = "free")+
  Theme1+
...
```{r}
#The temperature indices from brGMGTs as proposed in Naafs et al., 2018 and Baxter et al., 2019
dat88 <- select(OGdata, age, SSTOBL, MAT5ME, `brGMGTI`, FHbr.BR, FHbr.BR.N, HMBT.ALL, HMBT, outlierCAPRI)
HSSTGOOD <- filter(dat88, !outlierCAPRI)
HSSTBAD <- filter(dat88, outlierCAPRI)
HSSTGOOD <- gather(HSSTGOOD, SSTOBL:HMBT, value = "temp", key = "proxy")
HSSTBAD <- gather(HSSTBAD, SSTOBL:HMBT, value = "temp", key = "proxy")


HSSTGOOD$key_f = factor(HSSTGOOD$proxy, levels = c("SSTOBL", "MAT5ME", "brGMGTI","HMBT.ALL", "HMBT", "FHbr.BR", "FHbr.BR.N"))
HSSTGOOD <- mutate(HSSTGOOD, facet_label = fct_recode(
        key_f, 
        "SSTOBL (O'Brien et al., 2017)" = "SSTOBL", 
        "MAT5me (De Jonge et al., 2019)" = "MAT5ME",
        "brGMGTI (Baxter et al., 2019)" = "brGMGTI", 
        "%brGMGT (Naafs et al., 2018)" = "FHbr.BR.N", 
        "%brGMGT (Baxter et al., 2019)" = "FHbr.BR",
        "HMBT (Naafs et al., 2018)" = "HMBT",
        "HMBTall (Sluijs et al., 2020)" = "HMBT.ALL"
))

HSSTBAD$key_f = factor(HSSTBAD$proxy, levels = c("SSTOBL", "MAT5ME", "brGMGTI","HMBT.ALL", "HMBT", "FHbr.BR", "FHbr.BR.N"))
HSSTBAD <- mutate(HSSTBAD, facet_label = fct_recode(
        key_f, 
        "SSTOBL (O'Brien et al., 2017)" = "SSTOBL", 
        "MAT5me (De Jonge et al., 2019)" = "MAT5ME",
        "brGMGTI (Baxter et al., 2019)" = "brGMGTI", 
...
ggplot()+
  geom_point(HSSTGOOD, mapping = aes(x = `age`, y = `temp`), size = 0.4)+
  geom_line(HSSTGOOD, mapping = aes(x = `age`, y = `temp`))+
  geom_point(HSSTBAD, mapping = aes(x = `age`, y = `temp`), shape = 4, color = "grey", size = 0.4)+
  facet_grid(facet_label~., scales = "free")+
  theme(strip.text.y = element_text(angle=0, size = 7, hjust = 0))+
  Theme1+theme(aspect.ratio = 0.25)

ggsave("12. GMGT temp.pdf")
