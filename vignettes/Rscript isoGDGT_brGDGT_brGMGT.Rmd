...
```{r}
dat6 <- select(OGdata, age, MBT5ME, "BIT", "IR", "NoRINGStetra")
dat6 <- mutate(dat6, "OUTLIERIR" = IR>0.5)
dat6 <- pivot_longer(dat6, MBT5ME, "BIT", "IR", "NoRINGStetra", names_to = "TYPE", values_to = "index")
...
```
# brGMGTs
This is an exploratory part of the data visualization. The sources/substrates and ecological affinities of brGMGTs are uncertain. Moreover it is unsure whether the relative abundance of these components (relative to the brGDGTs or isoGDGTs) and the composition of brGMGTs chas any relationship to (paleo) environment, and to what extent this varies regionally, or per substrate. However, temperature seems to be a correlating factor. So let us explore.
First we investigate the relative abundance of brGMGTs to MAT, and then to SST.
##which brGMGTs correlate in fractional abundance to SST or MAT?
```{r}
dat10 <- select(OGdata, MBT5ME, `MAT5ME`, `OUTLIERMBT`, `brGMGTI`, `outlierCAPRI`, `SSTLIN`, `FH1048.Hbr`, `FH1034a.Hbr`, `FH1034b.Hbr`, `FH1034c.Hbr`, `FH1020a.Hbr`, `FH1020b.Hbr`, `FH1020c.Hbr`)
dat10 <- filter(dat10, !MBT5ME==1)
dat10r <- pivot_longer(dat10, `FH1048.Hbr`:`FH1020c.Hbr`, values_to = "FA", names_to = "HbrGDGT")
...
ggplot() +
    geom_point(dat10r, mapping = aes(x = SSTLIN, y = FA, shape = outlierCAPRI, color = outlierCAPRI), size = 0.2) +
    scale_shape_manual(values = c(16,4)) +
    scale_color_manual(values = c("black", "red")) +
    facet_grid(HbrGDGT~., scales = "free")
+
...
```
## We can look at how well brGMGTI' (Baxter et al., 2019) correlates to GDGT-based SST or MAT
```{r}
dat11 <- select(OGdata, SSTLIN, BIT, MBT5ME, MAT5ME, OUTLIERMBT, outlierCAPRI, `brGMGTI`)
...
MATGOOD <- pivot_longer(MATGOOD, MAT5ME, values_to = "temp", names_to = "proxy")
MATBAD <- pivot_longer(MATBAD, MAT5ME, values_to = "temp", names_to = "proxy")
SSTGOOD <- pivot_longer(SSTGOOD, SSTLIN, values_to = "temp", names_to = "proxy")
SSTBAD <- pivot_longer(SSTBAD, SSTLIN, values_to = "temp", names_to = "proxy")
...
#ggsave("11. GMGTI.pdf")
```
## We investigate the correlation of soil- and peat-based indicesfrom brGMGTs (Baxter et al., 2019; Naafs et al., 2017) to SST and MAT:
```{r}
dat88 <- select(OGdata, age, SSTLIN, MAT5ME, `brGMGTI`, FHbr.BR, FHbr.BR.N, HMBT.ALL, HMBT, outlierCAPRI)
HSSTGOOD <- filter(dat88, !outlierCAPRI)
HSSTBAD <- filter(dat88, outlierCAPRI)
HSSTGOOD <- pivot_longer(HSSTGOOD, SSTLIN:HMBT, values_to = "temp", names_to = "proxy")
HSSTBAD <- pivot_longer(HSSTBAD, SSTLIN:HMBT, values_to = "temp", names_to = "proxy")


HSSTGOOD$key_f = factor(HSSTGOOD$proxy, levels = c("SSTLIN", "MAT5ME", "brGMGTI","HMBT.ALL", "HMBT", "FHbr.BR", "FHbr.BR.N"))
HSSTGOOD <- mutate(HSSTGOOD, facet_label = fct_recode(
        key_f,
        "SSTLIN (O'Brien et al., 2017)" = "SSTLIN",
...
HSSTBAD$key_f = factor(HSSTBAD$proxy, levels = c("SSTLIN", "MAT5ME", "brGMGTI","HMBT.ALL", "HMBT", "FHbr.BR", "FHbr.BR.N"))
...
ggplot() +
  ...
  facet_grid(facet_label~., scales = "free") +
  theme(strip.text.y = element_text(angle=0, size = 7, hjust = 0), aspect.ratio = 0.25)
